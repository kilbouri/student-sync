...
namespace StudentSync::Common::Messages {
	struct StreamFrame {
		...
		StreamFrame(DisplayCapturer::Format format, std::vector<byte> imageData) : format{ format }, imageData{ imageData } {}
		...
		static std::optional<StreamFrame> FromNetworkMessage(const NetworkMessage& netMessage) noexcept {
